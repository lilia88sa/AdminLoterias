...
//        $em->flush();
        ...
        $lastrxg=$this->getDoctrine()->getRepository(Juego::class)->lastResultsXgame();
        $numsPrem=$this->getDoctrine()->getRepository(NumeroPremiado::class)->allNumPrem();
       // $numsPremFecha=$this->getDoctrine()->getRepository(NumeroPremiado::class)->allNumPremFecha();
		//dump($numsPremFecha);die;
		//dump($numsPrem);die;
        //$numnsPrem=$this->getDoctrine()->getRepository(NumeroPremiado::class)->alllNumPrem();
        $numnsPrem="{a0145:45,a24874:47}";
		//$f='{_23:258}';
		//$ft=($this->getDoctrine()->getRepository(NumeroPremiado::class)->alllNumPrem_1());

		//dump($numnsPrem);die;


        return $this->render('front/home.html.twig',['juegos'=>$games,'client'=>$client,'results'=>$last_results,
            'form'=>$form->createView(),'nums'=>$numsPrem,/*'numsfecha'=>$numsPremFecha,*/'numsx'=>$numnsPrem,'lastrxg'=>$lastrxg
        ]);
    }
...
//        dump($seccion);die();
 ...
 WHERE oc.name like '%covid-19%' and p.publish=1 order by p.createdAt desc ")->setMaxResults(1)->getResult();
    ...
    public function football_divs(){

        $d=file('https://www.laliga.com/laliga-santander/clasificacion');
        $r=explode('<script id="__NEXT_DATA__" type="application/json">',$d[count($d)-1]);
        $r1=explode('</script>',$r[1]);
        $result=json_decode($r1[0],true);

		$d=file('https://www.laliga.com/laliga-smartbank/clasificacion');
        $r=explode('<script id="__NEXT_DATA__" type="application/json">',$d[count($d)-1]);
        $r1=explode('</script>',$r[1]);
        $result1=json_decode($r1[0],true);
    ...
    /**
     * @Route("pantallas/{id}", name="pantallas")
     */
    public function pantallasAction(Request $request, $id): Response
    {

        $games=$this->getDoctrine()->getRepository(Juego::class)->findBy([], ['fecha_bote' => 'ASC']);
        $client=$this->getDoctrine()->getRepository(Cliente::class)->find($id);
        $last_results=$this->getDoctrine()->getRepository(Juego::class)->lastResultXgame();
        $last_5_results=$this->getDoctrine()->getRepository(Juego::class)->last5Results();

        $lastresult_eurom = $this->getDoctrine()->getRepository(Juego::class)->lastResultEurom();
        $lastresult_nacional = $this->getDoctrine()->getRepository(Juego::class)->lastResultNac();
        $lastresult_primitiva = $this->getDoctrine()->getRepository(Juego::class)->lastResultPrimitiva();
        $lastresult_bonoloto = $this->getDoctrine()->getRepository(Juego::class)->lastResultBonoloto();
        $lastresult_gordo = $this->getDoctrine()->getRepository(Juego::class)->lastResultGordo();
        $lastresult_lototurf = $this->getDoctrine()->getRepository(Juego::class)->lastResultLototurf();
        $lastresult_quintuple = $this->getDoctrine()->getRepository(Juego::class)->lastResultQuintuple();
        $lastresult_quinigol = $this->getDoctrine()->getRepository(Juego::class)->lastResultQuinigol();
        $lastresult_quiniela = $this->getDoctrine()->getRepository(Juego::class)->lastResultQuiniela();

        $last10result_eurom = $this->getDoctrine()->getRepository(Juego::class)->last10ResultEurom();
        $last10result_nacional = $this->getDoctrine()->getRepository(Juego::class)->last10ResultNac();
        $last10result_primitiva = $this->getDoctrine()->getRepository(Juego::class)->last10ResultPrimitiva();
        $last10result_bonoloto = $this->getDoctrine()->getRepository(Juego::class)->last10ResultBonoloto();
        $last10result_gordo = $this->getDoctrine()->getRepository(Juego::class)->last10ResultGordo();
        $last10result_lototurf = $this->getDoctrine()->getRepository(Juego::class)->last10ResultLototurf();
        $last10result_quintuple = $this->getDoctrine()->getRepository(Juego::class)->last10ResultQuintuple();
        $last10result_quinigol = $this->getDoctrine()->getRepository(Juego::class)->last10ResultQuinigol();
        $last10result_quiniela = $this->getDoctrine()->getRepository(Juego::class)->last10ResultQuiniela();


        $football_divs=$this->football_divs();
        $_1ra_div=$football_divs[0];
        $_2da_div=$football_divs[1];
        /*dump($_1ra_div);
        dump($_2da_div);
        die;*/


        return $this->render('front/pantallas/pantalla.html.twig',
            [   'juegos'=>$games,
                'client'=>$client,
                'results'=>$last_results,
                'last_5'=>$last_5_results,
                'lastresult_eurom' => $lastresult_eurom,
                'lastresult_nacional' => $lastresult_nacional,
                'lastresult_primitiva' => $lastresult_primitiva,
                'lastresult_bonoloto' => $lastresult_bonoloto,
                'lastresult_gordo' => $lastresult_gordo,
                'lastresult_lototurf' => $lastresult_lototurf,
                'lastresult_quintuple' => $lastresult_quintuple,
                'lastresult_quinigol' => $lastresult_quinigol,
                'lastresult_quiniela' => $lastresult_quiniela,
                'last10result_eurom' => $last10result_eurom,
                'last10result_nacional' => $last10result_nacional,
                'last10result_primitiva' => $last10result_primitiva,
                'last10result_bonoloto' => $last10result_bonoloto,
                'last10result_gordo' => $last10result_gordo,
                'last10result_lototurf' => $last10result_lototurf,
                'last10result_quintuple' => $last10result_quintuple,
                'last10result_quinigol' => $last10result_quinigol,
                'last10result_quiniela' => $last10result_quiniela,
                '_1ra_div' => $_1ra_div,
                '_2da_div' => $_2da_div
    ...
    public function tvAction(Request $request, $id): Response
    {
        $em = $this->getDoctrine()->getManager();
        $games=$this->getDoctrine()->getRepository(Juego::class)->findBy([], ['fecha_bote' => 'ASC']);
        $client=$this->getDoctrine()->getRepository(Cliente::class)->find($id);
        $consulta1 = $em->createQuery("SELECT c FROM  App\Entity\Section c JOIN  c.orderClasification oc JOIN c.cliente cl
            WHERE oc.name like '%carteles%' and cl.id = :id")->setMaxResults(1)->setParameters(['id' => $id])->getResult();
        $carteles=reset($consulta1);
        ...
        return $this->render('front/pantallas/televisor.html.twig',
            [   'juegos'=>$games,
                'client'=>$client,
                'results'=>$last_results,
                'carteles'=>$carteles,
